...
/**
 * TFB: term frequency (Bigrams).
 * 
 * This feature is used to score a sentence using the frequencies of consecutive
 * pair of terms in a text.
 * 
...
public class TFB implements Feature {
	...
	public void train(List<Object> trainParam) {
		...
		//Begin: calculate bigrams' frequencies in each class of topics
		for (int classID = 0; classID < classes.size(); classID++) {
			...
			for (int sentID : classes.get(classID)) {
				...
				for (String word : sentWords.get(sentID)) {
					...
					int value = (classIWordsFreq.containsKey(biWord)) ? classIWordsFreq
							.get(biWord) + 1 : 1;
		...
		//End: calculate bigrams' frequencies in each class of topics
